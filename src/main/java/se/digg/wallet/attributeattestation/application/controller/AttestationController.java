...
public class AttestationController {
  ...
  @GetMapping("/{id}")
  public ResponseEntity<Attestation> getAttestation(@PathVariable final UUID id) {
    return attestationService.getAttestationById(id).map(ResponseEntity::ok)
        .orElse(ResponseEntity.notFound().build());
  }

  @PostMapping()
  public ResponseEntity<Attestation> saveAttestation(
      @Valid @RequestBody CreateAttestationRequest attestationRequest) {
